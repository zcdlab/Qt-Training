    ...
    if(myCom->bytesAvailable() >=41 )
      {
        ...
        Human=buf.mid(1,1);
        if(Human[0]=='0')
            ui->Human_lineEdit->setText("未检测到人接近");
        else
            ui->Human_lineEdit->setText("Warning!检测到人接近");

        Fire=buf.mid(2,1);
        if(Fire[0]=='0')
            ui->Fire_lineEdit->setText("未检测到火焰");
        else
            ui->Fire_lineEdit->setText("Warning!检测到火焰");

        Sound=buf.mid(3,1);
        if(Sound[0]=='0')
            ui->Sound_lineEdit->setText("未检测到声音");
        else
            ui->Sound_lineEdit->setText("Warning!检测到声音");

        Temp=buf.mid(5,2);
        ui->Temp_lcdNumber->display(Temp);

        Humidity=buf.mid(8,2);
        ...
        MQ2=buf.mid(17,5);
        ui->MQ2_lcdNumber->display(MQ2);

        MQ7=buf.mid(23,5);
        ui->MQ7_lcdNumber->display(MQ7);
