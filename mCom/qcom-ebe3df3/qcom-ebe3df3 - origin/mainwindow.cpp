...
{
    mainAction=new QAction(tr("变色灯GUI主界面"),this);
    proAction=new QAction(tr("串口调试界面"),this);
    quitAction=new QAction(tr("退出软件"),this);

    mainMenu=menuBar()->addMenu(tr("切换界面"));
    mainMenu->addAction(mainAction);
    mainMenu->addAction(proAction);
    mainMenu->addSeparator();
    mainMenu->addAction(quitAction);

    aboutMenu=menuBar()->addMenu(tr("使用说明"));

    connect(mainAction,SIGNAL(triggered()),this,SLOT(on_actionMainMenu_triggered()));
...
void MainWindow::on_RED_pushButton_2_clicked()
{
    sendMessege("000255000");
}

void MainWindow::on_RED_pushButton_3_clicked()
{
