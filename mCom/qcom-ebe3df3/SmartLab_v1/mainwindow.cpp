...
void MainWindow::startInit(){
    ...
    ui->Temp_lcdNumber->display("00");
    ui->Humidity_lcdNumber->display("00");
...
void MainWindow::on_Red_spinBox_valueChanged(const QString &arg1)
{
   ...
   Red_SpinBox=argBuf;
   ALL_SpinBox=Blue_SpinBox.append(Green_SpinBox).append(Red_SpinBox);
...
{
   //Blue_SpinBox=ui->Blue_spinBox->text();
   //Green_SpinBox=ui->Green_spinBox->text();
   //Red_SpinBox=ui->Red_spinBox->text();
   //ALL_SpinBox=Blue_SpinBox.append(Green_SpinBox).append(Red_SpinBox);
   sendMessege(ALL_SpinBox);
}
